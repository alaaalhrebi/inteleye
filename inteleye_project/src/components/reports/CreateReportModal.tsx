...
export default function CreateReportModal({
  ...
  const branchPlatforms = useMemo(
    () =>
      platforms.filter(
        (platform) => platform.branchId === Number(branchId)
      ),
    [branchId, platforms]
  );
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!branchId || !platformId || !periodStart || !periodEnd) {
    ...
    const latestAllowed = new Date(`${periodStart}T00:00:00Z`);
    latestAllowed.setUTCFullYear(latestAllowed.getUTCFullYear() + 1);
    if (new Date(`${periodEnd}T00:00:00Z`) > latestAllowed) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.message || "تعذر إرسال طلب التقرير");
        return;
      }
      if (data.request_id === undefined || String(data.request_id).trim() === "") {
        setError("استجابة خدمة التقارير غير مكتملة");
        return;
      }
      await onAccepted({
        message: data.message || "تم استلام طلب التقرير وبدأت معالجته.",
    ...
    >
      <div className="max-h-[95vh] w-full overflow-y-auto rounded-t-[2rem] bg-white p-5 shadow-2xl sm:max-w-2xl sm:rounded-[2rem] sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-bold text-[#895159]">تقرير عند الطلب</p>
            <h2
            ...
            <p className="mt-2 leading-7 text-gray-500">
              اختر فرعًا ومنصة واحدة وفترة لا تتجاوز سنة.
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={submit} className="mt-7 space-y-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="المنصة">
              ...
              >
                <option value="">
                  {!branchId ? "اختر الفرع أولًا" : "اختر منصة واحدة"}
                </option>
          ...
          {branchId && branchPlatforms.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="إلى تاريخ" icon={<CalendarDays size={16} />}>
              <input
                required
                type="date"
                value={periodEnd}
          ...
          <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
            ...
            >
              ...
              {submitting ? "جارٍ الإرسال..." : "إنشاء التقرير"}
            </button>
          </div>
        </form>
      </div>
